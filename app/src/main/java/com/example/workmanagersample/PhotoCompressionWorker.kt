...
class PhotoCompressionWorker(
    ...
    override suspend fun doWork(): Result {
        return withContext(Dispatchers.IO) {
            ...
            val compressionThresoldInBytes = params.inputData.getLong(
                KEY_COMPRESSION_THRESHOLD, 0L
            )
            val uri = Uri.parse(stringUri)
            // read the bytes from the image which is behind that uri
            ...
            } while (outputBytes.size > compressionThresoldInBytes && quality > 5)
    ...
    companion object {
        ...
        const val KEY_COMPRESSION_THRESHOLD = "KEY_CONTENT_THRESOLD"
        const val KEY_RESULT_PATH = "KEY_CONTENT_PATH"
    }
}